...
function registerDpkiTo(app_agent_id) {
  var z = commit("app_agent_id_link", { Links: [{ Base: App.Agent.Hash, Link: app_agent_id, Tag: "app_agent_id" }] });
...
function registerDpkiKeyTo(app_agent_id) {
  var keyRegistration = getLinks(App.Agent.Hash, "keyRegistration", { Load: true });
  debug("keyRegistration: " + keyRegistration.length)
  if (keyRegistration.length !== 0) {
